        ...
        prev.map((r) =>
          r.id === row.id
            ? {
                ...
                repliedByName: message.repliedByUser?.name ?? null,
              }
            : r
        )
      );
            ...
            <div className="mt-3 space-y-3">
              ...
              {row.replyMessage ? (
                <div className="rounded-md border border-green-100 bg-green-50 p-3 text-sm">
                  <p className="mb-1 text-xs font-medium text-green-700">
                    Réponse envoyée
                    {row.repliedAt ? ` le ${format(new Date(row.repliedAt), "d MMMM yyyy à HH:mm", { locale: fr })}` : ""}
                    {row.repliedByName ? ` par ${row.repliedByName}` : ""}
                  </p>
                  <p className="whitespace-pre-wrap text-ink-700">{row.replyMessage}</p>
                </div>
              ) : null}
              ...
              <div className="space-y-2">
                <Textarea
                  ...
                  placeholder={row.replyMessage ? "Envoyer une nouvelle réponse..." : `Répondre à ${row.fullName}...`}
